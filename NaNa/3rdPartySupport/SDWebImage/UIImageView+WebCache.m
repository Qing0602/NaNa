 ...
 *
 * Purpose：To start the HTTPRequest immediately
 *****************************/
// 用来开始所有缓存的请求
- (void)setImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholder success:(SDWebImageSuccessBlock)success failure:(SDWebImageFailureBlock)failure immediately:(BOOL)isImmediately {
    SDWebImageManager *manager = [SDWebImageManager sharedManager];
    ...
    if (url)
    {
        [manager downloadWithURL:url delegate:self options:0 userInfo:nil success:success failure:failure immediately:isImmediately];
    }
}
/*****************************/
#endif
