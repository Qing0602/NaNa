...
- (void)downloadWithURL:(NSURL *)url delegate:(id)delegate options:(SDWebImageOptions)options userInfo:(NSDictionary *)userInfo success:(SDWebImageSuccessBlock)success failure:(SDWebImageFailureBlock)failure
{
    ...
    NSDictionary *info = [NSDictionary dictionaryWithObjectsAndKeys:
                          ...
                          successCopy, @"success",
                          failureCopy, @"failure",
                          [NSNumber numberWithBool:YES], @"immediately",
                          nil];
...
- (void)imageCache:(SDImageCache *)imageCache didNotFindImageForKey:(NSString *)key userInfo:(NSDictionary *)info
{
    ...
    if (!downloader)
    {
        ...
        /*****************************
         * Modified By SuYing: Mar 5th. 2013
         *
         * Purpose：Judge whether need download immediately
         *****************************/
        // 用来开始所有缓存的请求
        if ([info objectForKey:@"immediately"]) {
 ...
 * Modified By SuYing: Mar 5th. 2013
 *
 * Purpose：To set whether delay to download
 *****************************/
// 是否立即下载
- (void)downloadWithURL:(NSURL *)url delegate:(id)delegate options:(SDWebImageOptions)options userInfo:(NSDictionary *)userInfo success:(SDWebImageSuccessBlock)success
                failure:(SDWebImageFailureBlock)failure immediately:(BOOL)isImmediately {
    // repeated logic from above due to requirement for backwards compatability for iOS versions without blocks
    ...
    NSDictionary *info = [NSDictionary dictionaryWithObjectsAndKeys:
                          ...
                          successCopy, @"success",
                          failureCopy, @"failure",
                          isImmediately ? [NSNumber numberWithBool:YES] : nil, @"immediately",
                          nil];
    SDWIRelease(successCopy);
    SDWIRelease(failureCopy);
    [[SDImageCache sharedImageCache] queryDiskCacheForKey:[self cacheKeyForURL:url] delegate:self userInfo:info];
 ...
 * Modified By SuYing: Mar 5th. 2013
 *
 * Purpose：To start all the HTTPRequest
 *****************************/
// 用来开始所有缓存的请求
- (void)startAll {
    for ( SDWebImageDownloader *downloader in downloaders) {
