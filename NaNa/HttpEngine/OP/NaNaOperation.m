...
@implementation NaNaOperation

-(NaNaOperation *) initOperation{
    self = [super init];
    if (nil != self) {
...
-(NaNaOperation *) initOperation:(id)delegate withActon : (SEL) action{
...
//    url = [NSURL URLWithString:urlStr];
    ...
    self.request = [NaNaHTTPRequest requestWithURL:url];
    self.request.requestCookies = [[NSMutableArray alloc] initWithObjects:[NaNaUIManagement sharedInstance].php, nil];
    [self configRequest];
}
...
-(void)setHttpRequestPostWithUrl:(NSString *)urlStr params:(NSDictionary*)params{
    ...
    self.dataRequest = [NaNaFormDataRequest requestWithURL:url];
...
-(void) startAsynchronous{
    if (self.dataRequest){
        [self.dataRequest setDelegate:[NaNaNetWorkService sharedInstance]];
        [[NaNaNetWorkService sharedInstance] startAsynchronous:self.dataRequest];
    }
    if (self.request) {
        [self.request setDelegate:[NaNaNetWorkService sharedInstance]];
        [[NaNaNetWorkService sharedInstance] startAsynchronous:self.request];
    }
}
